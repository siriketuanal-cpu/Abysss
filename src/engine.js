...
const sync=()=>{const m=model();if(!m)return null;let changed=false;m.timers=m.timers.map(t=>{const n=AbysssModel.recover(t);if(n.current!==t.current||n.start!==t.start||n.running!==t.running)changed=true;return n});if(changed)save(m);window.dispatchEvent(new CustomEvent('abysss:modelchange'));return m};
...
let loop=0;
const tick=()=>{if(document.visibilityState==='visible'){const m=sync();if(m?.timers.some(t=>t.running)){clearTimeout(loop);loop=setTimeout(tick,1000)}}};
const resume=()=>{sync();clearTimeout(loop);loop=setTimeout(tick,1000)};
