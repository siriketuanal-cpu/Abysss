...
let loop=0;
const tick=()=>{if(document.visibilityState!=='visible')return;const m=sync();if(m?.timers.some(t=>t.running)){clearTimeout(loop);loop=setTimeout(tick,1000)}};
const resume=()=>{sync();clearTimeout(loop);loop=setTimeout(tick,1000)};
const touchCard=e=>{const card=e.target.closest?.('[data-timer-id]');if(!card||e.target.closest('[data-timer-input]'))return;sync();window.AbysssView?.render()};
...
document.addEventListener('pointerup',touchCard,{passive:true});
