...
const dotCard=t=>{const r=AbysssModel.recover(t),phase=AbysssModel.phase(r),next=phase===null?null:AbysssModel.STEP-phase;const available=Math.floor(r.current/40);const card=document.createElement('article');card.className='abyss-card dot-card';card.dataset.timerId=r.id;card.innerHTML=`<div class="state-head"><input class="abyss-name" data-timer-input data-timer-id="${esc(r.id)}" value="${esc(r.name||'Dot Abyss')}" aria-label="タイマー名"><span class="state-tag">DOT ABYSS</span></div><div class="state-main"><strong>${r.current}</strong><small>/ ${r.max}</small></div><div class="state-meta"><span>${r.current?'次の1回復':'回復開始'}</span><b>${next===null?'—':fmt(next)}</b></div><div class="state-foot"><span>${available} × 40</span><button type="button" data-action="consume40" data-id="${esc(r.id)}">使い切る</button></div>`;return card};
