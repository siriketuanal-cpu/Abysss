...
function buildJs(outputName, sources) {
  ...
  const output = execFileSync(esbuild, [
    '--minify-syntax', '--minify-whitespace', '--legal-comments=none', '--charset=utf8'
...
function buildCss(outputName, source) {
  const output = execFileSync(esbuild, [
    '--minify', '--legal-comments=none', '--charset=utf8', '--loader=css'
  ], { input: source, encoding: 'utf8', stdio: ['pipe', 'pipe', 'inherit'] });
  writeOutput(outputName, output);
}

const cssLines = read('src/styles.css').split('\n');
const primaryCss = [...cssLines.slice(0, 471), ':root{--timer-pop-system-font:ui-rounded,"Arial Rounded MT Bold","Trebuchet MS",Roboto,system-ui,sans-serif;}', 'input,textarea{caret-color:transparent!important;}'].join('\n');
const gamesCss = [...cssLines.slice(471), '#gamesAll{contain:layout paint style;}'].join('\n');

buildJs('dist/app.min.js', ['src/timer-engine-v2.js', 'src/core.js', 'src/dotabyss.js', 'src/bootstrap.js']);
